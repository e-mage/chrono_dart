...
double? parseNumberPattern(String match) {
  ...
  } else if (RegExp(r'несколько').hasMatch(nmb)) {
    return 3;
  } else if (RegExp(r'пол').hasMatch(nmb)) {
    return 0.5;
  } else if (RegExp(r'пар').hasMatch(nmb)) {
    return 2;
  } else if (nmb == "") {
    return 1;
  }
...
int? parseYear(String match) {
  if (RegExp(r'(год|года|г|г.)', caseSensitive: false).hasMatch(match)) {
    // Regular year
    match = match.replaceFirst(RegExp(r'(год|года|г|г.)', caseSensitive: false), "");
    //return (int.tryParse(match) ?? 0);
  }

  if (RegExp(r'(до н.э.|до н. э.)', caseSensitive: false).hasMatch(match)) {
    // Before Christ, Before Common Era
    match = match.replaceFirst(RegExp(r'(до н.э.|до н. э.)', caseSensitive: false), "");
    return -(int.tryParse(match) ?? 0);
  }

  if (RegExp(r'(н. э.|н.э.)', caseSensitive: false).hasMatch(match)) {
    // Anno Domini, Common Era
    match = match.replaceFirst(RegExp(r'(н. э.|н.э.)', caseSensitive: false), "");
    return int.tryParse(match) ?? 0;
  }
...
final TIME_UNITS_NO_ABBR_PATTERN = repeatedTimeunitPattern(
    "(?:(?:около|примерно)\\s{0,3})?", SINGLE_TIME_UNIT_NO_ABBR_PATTERN);
